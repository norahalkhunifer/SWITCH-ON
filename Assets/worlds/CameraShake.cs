...
public class CameraShake : MonoBehaviour {
	/*
	public float power =0.7f,duration=1.0f,slowdown=1.0f;
	...
	void Update (){
		if (shaking) {
			if (duration > 0) {
				camera.localPosition = starting + Random.insideUnitSphere * power;
				duration -= Time.deltaTime * slowdown;
			} else {
				shaking = false;
				duration = initialDuration;
				camera.localPosition = starting;
			}
